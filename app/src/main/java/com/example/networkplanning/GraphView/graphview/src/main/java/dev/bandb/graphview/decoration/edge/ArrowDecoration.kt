...
open class ArrowDecoration constructor(private val linePaint: Paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
     ...
     * @param y2
     */
    protected fun drawTriangle(canvas: Canvas, paint: Paint?, x1: Float, y1: Float, x2: Float, y2: Float, value: Int = -1): FloatArray {
        ...
        if(value!=-1) {
            ...
            canvas.drawCircle((x1 + x2) / 2, (y1 + y2) / 2,23f,paint!!)
            paint.color=Color.BLACK
            paint!!.textSize = 35f
            canvas.drawText(" $value", (x1 + x2) / 2 - 20f, (y1 + y2) / 2 +10f, paint!!)
        }
    ...
    companion object {
        ...
        private const val ARROW_LENGTH = 25f
    }
}
